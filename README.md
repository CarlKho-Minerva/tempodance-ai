...
- **Practice can be gated step-by-step or full-routine.** Step-by-step freezes the source video, audio, and coach overlay on every count until the learner holds at least 82% focused-pose accuracy, while upper body, lower body, and combined passes keep cognitive load bounded. Full routine enables continuous complete-movement playback.
...
The reference panel plays the locally packaged copy of `https://www.youtube.com/shorts/jrUsvBKemBU` with its audio and extracted pose overlay. Leave **Step by step** selected to freeze at each count until the focused pose reaches 82%, and use **Next** to move from upper body to lower body, then to the combined move. Select **Full routine** for continuous playback.
