...
- **Practice has three distinct plans.** Step by step moves manually through upper, lower, and combined passes; Progression gating freezes each source count until the learner holds at least 65% focused-pose accuracy; Full routine plays continuously. Upper, lower, and combined isolation remain selectable in every plan.
- **The Tempo agent panel is collapsible.** Hide it to expand the synchronized video and learner views, then restore it without resetting the session.
...
The reference panel plays the locally packaged copy of `https://www.youtube.com/shorts/jrUsvBKemBU` with its audio and extracted pose overlay. Use **Step by step** for manual Upper → Lower → Together practice, **Progression gating** to freeze each count until focused accuracy reaches 65%, or **Full routine** for continuous playback. The Upper, Lower, and Together controls can isolate rendering and scoring in all three plans.
